...
size_t correct_algo(maze m) {
    ...
    while (x != SIZEX - 2 || y != SIZEY - 2) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (nx >= 0 && nx < SIZEX && ny >= 0 && ny < SIZEY && visited[nx][ny] != MX &&
                visited[nx][ny] <= min_visits) {
...
size_t algo_2(maze m) {
    ...
    while (x != SIZEX - 2 || y != SIZEY - 2) {
        ...
        if (min_visits == visited[x + dx[cur_direction]][y + dy[cur_direction]]) {
            ...
            int min_direction = min_visits == visited[x + 1][y]   ? 0
                                : min_visits == visited[x][y + 1] ? 1
                                : min_visits == visited[x - 1][y] ? 2
                                                                  : 3;
...
class ResearcData : public ResearchDataBase {
  public:
    ...
    string to_string() override { return data.to_string(); }
    void from_string(string s) override { data.from_string(s); }
};
